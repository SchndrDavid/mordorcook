...
the browser on your desk always show the same library.
...
- `data/mordorcook.db` — SQLite database holding recipes, photo metadata and
  the shopping list.
...
Nothing is stored in the browser except appearance, text size, any timers
currently running, which ingredients you have ticked off, and the step you had
reached in cooking mode. All of that is per-device by design.
...
Run it on a private network, not on the open internet.
